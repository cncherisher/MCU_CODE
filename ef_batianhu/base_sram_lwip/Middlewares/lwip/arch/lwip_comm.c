...
void lwip_dhcp_process(struct netif *netif)
{
    ...
    uint32_t gw = 0;
    struct dhcp *dhcp;
    uint8_t s_ip_addr[20];                  	/* store static ip address */
	  uint8_t s_netmask[20];                  /* store static ip netmask */
	  uint8_t s_gw[20];                  /* store static ip gateway */
    g_lwipdev.dhcpstatus = 1;           		/* DHCP flag = 1 */
    ...
    switch (g_lwip_dhcp_state)
    {
        ...
        {
            /* set ipaddr, netmask, gateway to zero */
            ...
            if (dhcp_supplied_address(netif)) 
            {
                ...
                if (ip != 0)
                {
                    ...
                    g_lwipdev.ip[0] = (uint8_t)(ip);
										printf("ip address from dhcp is: %d.%d.%d.%d\r\n", g_lwipdev.ip[0], g_lwipdev.ip[1], g_lwipdev.ip[2], g_lwipdev.ip[3]);
                    /* 解析通过DHCP获取到的子网掩码地址 */
                    ...
                    g_lwipdev.netmask[0] = (uint8_t)(netmask);
										printf("netmask from dhcp is: %d.%d.%d.%d\r\n", g_lwipdev.netmask[0], g_lwipdev.netmask[1], g_lwipdev.netmask[2], g_lwipdev.netmask[3]);
                    /* 解析出通过DHCP获取到的默认网关 */
                    ...
                    g_lwipdev.gateway[0] = (uint8_t)(gw);
										printf("gateway from dhcp is: %d.%d.%d.%d\r\n", g_lwipdev.gateway[0], g_lwipdev.gateway[1], g_lwipdev.gateway[2], g_lwipdev.gateway[3]);
                }
            }
            else
            {
                ...
                {
                    printf ("DHCP failed, setting static address......\r\n");
                    ...
                    netif_set_addr(netif, &g_lwip_netif.ip_addr, &g_lwip_netif.netmask, &g_lwip_netif.gw);
										sprintf((char *)s_ip_addr, "%s", ip4addr_ntoa((const ip4_addr_t *)&netif->ip_addr));
										sprintf((char *)s_netmask, "%s", ip4addr_ntoa((const ip4_addr_t *)&netif->netmask));
										sprintf((char *)s_gw, "%s", ip4addr_ntoa((const ip4_addr_t *)&netif->gw));
										printf ("Static IP address is: %s\r\n", s_ip_addr);
										printf ("Static netmask is: %s\r\n", s_netmask);
										printf ("Static gateway is: %s\r\n", s_gw);
                }
            }
        }
        break;
